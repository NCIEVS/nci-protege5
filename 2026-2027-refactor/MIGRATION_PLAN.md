...
- ~~Create the `NCIEVS/owl-rdf-io` GitHub repo and push `main` + `2026-2027-refactor`~~ **Done (2026-09-18, by human):** the remote repo was created and both branches pushed, so `build.sh` can now clone `owl-rdf-io`. Step 1 is fully unblocked.
...
- **Per-class commit gate** (`140fd9ba`, Decision #2): `ConflictDetectionFilter.commit` no longer rejects on `base < head`. When behind, it derives the entities its own changes touch (subject of each add/remove axiom) and the entities changed in `(base, head]` (via `changeService.getChanges`) and rejects only on overlap — so modelers editing disjoint entities commit concurrently, and real same-entity conflicts still throw `OutOfSyncException` (now naming the entities). Subject extraction is subject-scoped for the dominant NCI axiom shapes and falls back to the full signature / global gate otherwise (over-reports at worst, never misses a conflict). Index-over-the-log is deferred; the interval is recomputed per commit for now.

Done (2026-09-18) — **folded in the EVS descriptor (Decision #4)**, a cross-repo change:
- **Client** (`nci-edit-tab` `b756961`): `NCIEditTab.putHistory` now accumulates the EVS descriptor (built with the client user id) into a `pendingEvsRecords` list instead of POSTing `putEVSHistory` after the commit. `doCommit`/`commitPowerDelete` compute the descriptors *before* creating the commit (recomputing fresh for a normal commit so a login-timeout retry doesn't double-record; batch mode keeps what `applyChanges` accumulated), attach them to the bundle, and clear the list only once the commit succeeds so a failed retry keeps them.
- **Wire + log + server** (`protege` `52faa842`): `RevisionMetadata` gains an `evsRecords` list (Java-serialized in the bundle, **persisted in the change log** via a new `evs.records` `BinaryOWLMetadata` attribute, encoded/decoded by `History.encodeEvsList`/`decodeEvsList` in both `ChangeHistoryImpl` and `ChangeHistoryUtils`). `HTTPChangeService.submitCommitBundle` appends the records to the `evs_history`/`current_evshistory` files in the **same request** that committed the changeset; a file-append failure is logged but never fails the commit — the log now holds the descriptors, so `evs_history` is a rebuildable projection. `ClientUtils.createCommit` gained an overload taking the EVS records.
- Legacy `LocalHttpClient.putEVSHistory` + the `EVS_REC` endpoint are left intact (dormant fallback). The commit-transport rewrite (finding #5, dropping Java serialization) is still separate and now unblocked from EVS concerns.

Not yet exercised in-app: needs a client+server redeploy and a create/split/merge/retire edit to confirm the `evs_history` file gets the record from the commit (and that the record is present in the log metadata).

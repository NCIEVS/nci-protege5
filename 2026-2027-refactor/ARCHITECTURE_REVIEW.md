...
- **Granularity:** this is global (one commit at a time per project), matching the v1 coarse-gate decision. Per-class version checks (letting non-conflicting commits run concurrently) are a later optimization; even then Virtuoso applies must stay in serialized revision order, so the boundary argument still holds.
